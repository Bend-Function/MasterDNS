...
Response: `{ "results": Ack[] }`, where each `Ack` is `{ "taskId": UUID, "status": "accepted" | "duplicate" | "stale" | "rejected" }`.

The batch contains 1 to 100 results. A duplicate is an idempotent acknowledgement. Stale and rejected observations may be retained for diagnostics but cannot update health state.
