...
The cloud and DNS control planes are the only fakes. Production EC2 and Cloudflare adapters, HTTP ingestion, scheduler/finalization, rotation persistence, publication and operations execute against them. Remote effects and an attempted-mutation journal live in the parent process and survive termination of actual worker child processes. The journal records requests before fake validation or effects, including rejected requests. Child environments contain only the explicit test connections and runtime paths; no cloud credentials or notification/webhook settings are inherited.
...
- Real reconcile and operation BullMQ workers publish both DNS bindings after exact-version candidate verification. The successful DNS step keeps its original ID, status and attempt count across SIGKILL before the second provider write; the attempted-request journal confirms restart tries only the missing write.
- SIGKILL immediately before cleanup preserves the pending cleanup record. The fixture seeds old published DNS records with TTL 120 and requests a new TTL of 60. It asserts that the computed old maximum is exactly 120, checks the cleanup deadline against the independent constant 120 + 60 seconds, and waits that full deadline in bounded polling intervals. After a second SIGKILL loses the release receipt, restart observes absence. Every release attempt must target only the old allocation, and the current allocation, address and interface attachment must remain intact.
...
- Shared lifecycle helpers handle redaction, subprocess capture, TLS files/requests, API bootstrap, enrollment and owned cleanup. The early-exit regression proves output is redacted while secret detection still fails the gate.
