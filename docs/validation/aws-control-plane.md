...
Initial Lightsail inspection uses only `GetInstance` for the explicit native instance name and, unless `MASTERDNS_AWS_E2E_LIGHTSAIL_STATIC_IP_NAME=none`, `GetStaticIp` for the explicit original static-IP name. After the journal exists, scoped reads permit only that persisted original identity and the deterministic candidate name/ARN/address from an applied allocation receipt. Detached-original and candidate-attached states are accepted only after the matching journal step was dispatched. Static-IP ARNs must share the instance ARN's partition, service, region, and account, and attachments may never point to another instance. The harness does not call `GetInstances` or `GetStaticIps` to discover matching resources.
