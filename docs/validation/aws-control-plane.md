...
| `MASTERDNS_AWS_E2E_LIGHTSAIL_IPV4_ADDRESS` | Lightsail IPv6 only: exact independent public IPv4 address |
...
The harness creates one attempt and persists its original normalized inventory and full production `CloudStep` plan before dispatch. Only an absent journal file initializes an attempt; `null`, `false`, `0`, an empty string, and every other loaded JSON value must pass journal validation and cannot fall back to initialization. Each journal replacement is written to a private temporary file, synced, renamed, and followed by a directory sync. Persistence errors abort before the corresponding mutation. On resume, the plan is rebuilt from the persisted original inventory and must exactly match action order, IDs, scope, and before snapshots. Before each mutation the harness rechecks the STS account and exact instance/interface scope. After each side effect it immediately persists the provider receipt, then reads the remote state through `observeDetails`. No next step is dispatched until the prior observation is `applied`.

Initial Lightsail inspection uses only `GetInstance` for the explicit native instance name and, unless `MASTERDNS_AWS_E2E_LIGHTSAIL_STATIC_IP_NAME=none`, `GetStaticIp` for the explicit original static-IP name. An IPv6 slot checks `MASTERDNS_AWS_E2E_ADDRESS` against `ipv6Addresses` while independently checking the instance's IPv4 and static-IP identity against `MASTERDNS_AWS_E2E_LIGHTSAIL_IPV4_ADDRESS`. After the journal exists, scoped IPv4 reads permit only the persisted original identity and the deterministic candidate name/ARN/address from an applied allocation receipt. A missing selected IPv6 is accepted only after disable dispatch; one new IPv6 is accepted only after enable dispatch, and becomes exact after its applied observation. Detached-original and candidate-attached IPv4 states are accepted only after the matching journal step was dispatched. Static-IP ARNs must share the instance ARN's partition, service, region, and account, and attachments may never point to another instance. The harness does not call `GetInstances` or `GetStaticIps` to discover matching resources.
