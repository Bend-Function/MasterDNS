...
export default function ProbesPage() {
  ...
  useEffect(() => { if (UI_PREVIEW) return; const timer = setInterval(() => { setClock(new Date()); void load(); }, 30_000); return () => clearInterval(timer); }, [load]);
  ...
  const active = probes?.filter((probe) => isProbeOnline(probe, UI_PREVIEW ? new Date(demoNow) : clock)).length ?? 0;
  ...
  return <ConsoleLayout><PageHeader title="外部探测点" description="管理 probe-agent/v1 节点、固定探测组与一次性安装凭据" actions={<><Button variant="secondary" icon={<UsersRound size={14} />} onClick={() => setGroupTarget("new")}>新建探测组</Button><Button icon={<Plus size={14} />} onClick={() => setCreateOpen(true)}>添加探测点</Button><IconButton label="刷新探测点" onClick={() => void load()}><RefreshCw size={16} /></IconButton></>} />
    <MetricStrip items={[{ label: "探测点", value: probes?.length ?? "-", detail: `${active} 个在线` }, { label: "IPv4 能力", value: probes?.filter((probe) => probe.capabilities.ipv4).length ?? "-", detail: "unavailable 不计故障" }, { label: "IPv6 能力", value: ipv6, detail: "按地址族独立上报" }, { label: "固定探测组", value: groups?.length ?? "-", detail: "成员变更产生新 Revision" }]} />
    {error && <div className="inline-error" role="alert">{error}</div>}
    {!probes || !groups ? <div className="surface"><LoadingState /></div> : <div className="content-grid"><section className="surface"><header className="surface-header"><div><h2>探测点</h2><p>能力来自最近一次 heartbeat；90 秒未上报视为离线</p></div><RadioTower size={16} /></header>{probes.length === 0 ? <EmptyState title="尚未创建外部探测点" /> : <div className="table-wrap"><table><thead><tr><th>名称</th><th>状态</th><th>能力</th><th>并发</th><th>版本</th><th>最近心跳</th><th aria-label="操作" /></tr></thead><tbody>{probes.map((probe) => <tr key={probe.id}><td><div className="table-primary"><strong>{probe.name}</strong><small className="mono">{probe.id}</small></div></td><td><StatusBadge value={probe.revokedAt ? "revoked" : !probe.enabled ? "disabled" : !probe.lastSeenAt ? "pending" : isProbeOnline(probe, UI_PREVIEW ? new Date(demoNow) : clock) ? "active" : "offline"} /></td><td><div className="capability-list"><span className={probe.capabilities.ipv4 ? "available" : "unavailable"}>IPv4 {probe.capabilities.ipv4 ? "可用" : "不可用"}</span><span className={probe.capabilities.ipv6 ? "available" : "unavailable"}>IPv6 {probe.capabilities.ipv6 ? "可用" : "不可用"}</span></div></td><td>{probe.reportedConcurrency} / {probe.maxConcurrency}</td><td>{probe.agentVersion ?? "尚未上报"}</td><td><RelativeTime value={probe.lastSeenAt} /></td><td><div className="row-actions"><Button variant="ghost" disabled={busyId === probe.id} onClick={() => void issueInstallToken(probe)}>{probe.lastSeenAt ? "重新注册" : "安装"}</Button>{probe.enabled && <IconButton label={`吊销 ${probe.name}`} disabled={busyId === probe.id} onClick={() => setRevokeTarget(probe)}><Ban size={15} /></IconButton>}</div></td></tr>)}</tbody></table></div>}</section>
