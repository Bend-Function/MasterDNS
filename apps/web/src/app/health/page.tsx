...
import { actualEndpointFamilies, healthPolicyDisplay, reconcileEndpointFamily } from "../../lib/health-target";
...
export default function HealthPage() {
  ...
  const beginEdit = (policy: AddressHealthPolicy) => setEditing({ kind: policy.slotId ? "slot" : "endpoint", id: policy.slotId ?? policy.endpointId!, family: policy.family, label: policy.cloudTarget ? `${cloudTargetLabel(policy.cloudTarget)} · ${cloudTargetAddresses(policy.cloudTarget)}` : `${policy.slotId ? "云槽位" : "节点"} ${shortId(policy.slotId ?? policy.endpointId!)}`, config: policy.config, policy });
  ...
  const displayStates = new Map(visiblePolicies?.map(policy => [policy.id, healthPolicyDisplay(policy, groups.find(group => group.id === policy.groupId), now)]));
  const healthy = [...displayStates.values()].filter(state => state.status === "healthy").length;
  const unknown = [...displayStates.values()].filter(state => state.status === "unknown").length;
  return <ConsoleLayout><PageHeader title="健康检查" description="本地与固定外部 Cohort 的 HTTP、HTTPS 和 TCP 检查" actions={<><Button icon={<Plus size={14} />} onClick={() => setTargetPickerOpen(true)}>配置地址策略</Button><IconButton label="刷新健康状态" onClick={() => void load()}><RefreshCw size={16} /></IconButton></>} />
    ...
    {policies === null ? <div className="surface"><LoadingState /></div> : <><section className="surface"><header className="surface-header"><div><h2>地址健康策略</h2><p>云槽位必须保留外部探测权威</p></div><label className="check-row"><input type="checkbox" checked={showHistory} onChange={event => setShowHistory(event.target.checked)} />显示历史地址策略</label><Activity size={16} /></header>{visiblePolicies?.length === 0 ? <EmptyState title="暂无当前地址健康策略" /> : <div className="table-wrap"><table><thead><tr><th>目标 / 地址</th><th>地址族</th><th>状态</th><th>最近决策</th><th>Cohort / 语义</th><th>阈值</th><th>证据有效期</th><th aria-label="操作" /></tr></thead><tbody>{visiblePolicies?.map((policy) => {
      const group = groups.find(candidate => candidate.id === policy.groupId);
      const size = policy.mode === "local" ? 1 : (group?.memberIds.length ?? 0) + (policy.mode === "mixed" ? 1 : 0);
      const preview = consensusPreview(policy.consensus, size);
      const display = displayStates.get(policy.id)!;
      return <tr key={policy.id}>
        <td><button className="table-link" onClick={() => beginEdit(policy)}><strong>{policy.cloudTarget ? cloudTargetLabel(policy.cloudTarget) : policy.slotId ? "云地址槽位" : "普通节点"}</strong><small className="mono">{policy.cloudTarget ? cloudTargetAddresses(policy.cloudTarget) : shortId(policy.slotId ?? policy.endpointId!)}</small></button></td>
        <td>IPv{policy.family}</td><td><div className="table-primary"><StatusBadge value={display.status} />{display.reason && <small>{display.reason}</small>}</div></td><td><StatusBadge value={display.decision} /></td>
        <td><div className="table-primary"><strong>{group?.name ?? (policy.mode === "local" ? "本地" : "探测组不可用")}</strong><small>{size} 票 · Q={policy.consensus.minimumValid}{preview.failureVotesRequired === null ? " · 指定点" : ` · F>=${preview.failureVotesRequired} · S>=${preview.successVotesRequired}`}</small></div></td>
        <td>{policy.failureThreshold} 次失败 / {policy.successThreshold} 次成功</td><td><RelativeTime value={policy.state?.evidenceExpiresAt} future /></td><td><div className="row-actions"><Button variant="ghost" onClick={() => void openEvidence(policy)}>投票明细</Button><IconButton label="编辑策略" onClick={() => beginEdit(policy)}><ExternalLink size={15} /></IconButton></div></td>
      </tr>;
    })}</tbody></table></div>}</section>
      <section className="surface"><header className="surface-header"><div><h2>Pool 检查概览</h2><p>按 Pool 实际采用的本地或外部探测结果汇总</p></div></header><div className="table-wrap"><table><thead><tr><th>Pool</th><th>状态</th><th>健康节点</th><th>间隔</th><th>最近协调</th><th aria-label="操作" /></tr></thead><tbody>{pools.map((pool) => <tr key={pool.id}><td><Link className="table-primary" href={`/pools/${pool.id}`}><strong>{pool.name}</strong><small>{pool.strategy}</small></Link></td><td><StatusBadge value={pool.enabled ? pool.state : "disabled"} /></td><td>{pool.healthyEndpointCount ?? 0} / {pool.endpointCount ?? 0}</td><td>{pool.checkIntervalSeconds}s</td><td><RelativeTime value={pool.lastReconciledAt} /></td><td><Link className="icon-button" href={`/pools/${pool.id}`} aria-label="打开 Pool"><ExternalLink size={15} /></Link></td></tr>)}</tbody></table></div></section></>}
...
function HealthTargetPicker({ open, accounts, pools, policies, onClose, onSelected }: { open: boolean; accounts: CloudAccount[]; pools: Pool[]; policies: AddressHealthPolicy[]; onClose: () => void; onSelected: (target: PolicyTarget) => void }) {
  ...
  const currentSlots = slots.filter(entry => entry.isCurrent !== false && entry.cloudTarget?.available !== false);
  const slot = currentSlots.find((entry) => entry.slot.id === slotId); const endpointCandidate = poolDetail?.endpoints.find((entry) => entry.id === endpointId); const endpointFamilies = endpointCandidate ? actualEndpointFamilies(endpointCandidate) : []; const selectedFamily = endpointCandidate ? reconcileEndpointFamily(family, endpointCandidate) : null; const endpoint = selectedFamily ? endpointCandidate : undefined; const endpointConfig = poolDetail?.healthChecks.find((check) => check.endpointId === endpointId) as HealthConfigRow | undefined;
  const submit = async () => { if (kind === "slot" && slot) { setLoading(true); setError(null); const request = generation.current.invalidate(); try { const policy = policies.find((item) => item.slotId === slot.slot.id) ?? null; const config = policy?.config ?? (UI_PREVIEW ? null : await api<HealthConfigRow | null>(`/v1/address-slots/${slot.slot.id}/health-config`)); if (generation.current.isCurrent(request)) onSelected({ kind, id: slot.slot.id, family: slot.slot.family, label: slot.cloudTarget ? `${cloudTargetLabel(slot.cloudTarget)} · ${cloudTargetAddresses(slot.cloudTarget)}` : slot.slot.name, config, policy }); } catch (value) { if (generation.current.isCurrent(request)) setError(message(value, "健康配置加载失败")); } finally { if (generation.current.isCurrent(request)) setLoading(false); } return; } if (kind === "endpoint" && endpoint && selectedFamily && poolDetail) { const policy = policies.find((item) => item.endpointId === endpoint.id && item.family === selectedFamily) ?? null; onSelected({ kind, id: endpoint.id, family: selectedFamily, label: `${poolDetail.pool.name} / ${endpoint.name}`, poolId: poolDetail.pool.id, config: policy?.config ?? endpointConfig ?? null, policy }); } };
