...
import { consensusPreview } from "../../lib/probe-policy";
...
export default function HealthPage() {
  ...
  const save = async ({ config, policy }: { config: HealthCheckConfig; policy: HealthPolicyInput }) => {
    ...
    try {
      ...
      if (!mutationGeneration.current.isCurrent(generation)) return;
      setSaving(false);
      if (value instanceof ApiError && value.status === 409) { await load(); throw new Error("配置已被其他操作更新，请按最新 Revision 重新提交"); }
      throw value;
    }
  };
...
function HealthTargetPicker({ open, accounts, pools, policies, onClose, onSelected }: { open: boolean; accounts: CloudAccount[]; pools: Pool[]; policies: AddressHealthPolicy[]; onClose: () => void; onSelected: (target: PolicyTarget) => void }) {
  ...
  const slot = slots.find((entry) => entry.slot.id === slotId); const endpoint = poolDetail?.endpoints.find((entry) => entry.id === endpointId); const endpointConfig = poolDetail?.healthChecks.find((check) => check.endpointId === endpointId) as HealthConfigRow | undefined;
  const submit = async () => { if (kind === "slot" && slot) { setLoading(true); setError(null); const request = generation.current.invalidate(); try { const policy = policies.find((item) => item.slotId === slot.slot.id) ?? null; const config = policy?.config ?? (UI_PREVIEW ? null : await api<HealthConfigRow | null>(`/v1/address-slots/${slot.slot.id}/health-config`)); if (generation.current.isCurrent(request)) onSelected({ kind, id: slot.slot.id, family: slot.slot.family, label: slot.slot.name, config, policy }); } catch (value) { if (generation.current.isCurrent(request)) setError(message(value, "健康配置加载失败")); } finally { if (generation.current.isCurrent(request)) setLoading(false); } return; } if (kind === "endpoint" && endpoint && poolDetail) { const policy = policies.find((item) => item.endpointId === endpoint.id && item.family === family) ?? null; onSelected({ kind, id: endpoint.id, family, label: `${poolDetail.pool.name} / ${endpoint.name}`, poolId: poolDetail.pool.id, config: policy?.config ?? endpointConfig ?? null, policy }); } };
  return <Dialog open={open} title="选择健康目标" onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>取消</Button><Button disabled={kind === "slot" ? !slot : !endpoint} onClick={submit}>继续配置</Button></>}><div className="policy-form"><div className="segmented"><button type="button" className={kind === "slot" ? "active" : ""} onClick={() => setKind("slot")}>云地址槽位</button><button type="button" className={kind === "endpoint" ? "active" : ""} onClick={() => setKind("endpoint")}>普通节点</button></div>{error && <div className="inline-error">{error}</div>}{kind === "slot" ? <><Field label="云账号"><select value={accountId} onChange={(event) => void chooseAccount(event.target.value)}><option value="">选择账号</option>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field label="云实例"><select value={instanceId} disabled={!accountId} onChange={(event) => void chooseInstance(event.target.value)}><option value="">选择实例</option>{instances.map((item) => <option key={item.instance.id} value={item.instance.id}>{item.instance.name ?? item.instance.externalId} / {item.instance.region}</option>)}</select></Field><Field label="地址槽位"><select value={slotId} disabled={!instanceId} onChange={(event) => setSlotId(event.target.value)}><option value="">选择槽位</option>{slots.map((entry) => <option key={entry.slot.id} value={entry.slot.id}>{entry.slot.name} / IPv{entry.slot.family} / {entry.currentAddress?.address ?? "暂无观测地址"}</option>)}</select></Field></> : <><Field label="Pool"><select value={poolId} onChange={(event) => void choosePool(event.target.value)}><option value="">选择 Pool</option>{pools.map((pool) => <option key={pool.id} value={pool.id}>{pool.name}</option>)}</select></Field><Field label="普通节点"><select value={endpointId} disabled={!poolId} onChange={(event) => setEndpointId(event.target.value)}><option value="">选择节点</option>{poolDetail?.endpoints.filter((item) => item.addressMode !== ("cloud" as Endpoint["addressMode"])).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field label="地址族"><select value={family} onChange={(event) => setFamily(event.target.value as "4" | "6")}><option value="4">IPv4</option><option value="6">IPv6</option></select></Field></>}{loading && <LoadingState />}</div></Dialog>;
}

function EvidenceView({ evidence, probes }: { evidence: { policy: AddressHealthPolicy; rounds: ProbeRound[]; stats: ProbeObservationStat[] }; probes: ProbeAgent[] }) { return <div className="evidence-view"><div className="vote-preview"><strong>有效结果门槛 Q={evidence.policy.consensus.minimumValid}</strong><span>unknown 与 unavailable 不会转成 failure；截止时按轮次固定成员计算。</span></div><section><h3>最近轮次</h3>{evidence.rounds.length === 0 ? <EmptyState title="暂无轮次证据" /> : evidence.rounds.slice(0, 20).map((round) => <div className="round-block" key={round.id}><header><span>#{round.sequence} · <span className="mono">{round.address}</span></span><StatusBadge value={round.consensusResult ?? "unknown"} /></header><div className="vote-grid">{round.observations.map((observation) => <div key={observation.id}><strong>{probes.find((probe) => probe.id === observation.probeId)?.name ?? shortId(observation.probeId)}</strong><StatusBadge value={observation.outcome === "unavailable" ? "unknown" : observation.outcome} /><small>{observation.outcome === "unavailable" ? "探测能力不可用" : `${Math.round(observation.latencyMs)} ms${observation.statusCode ? ` · HTTP ${observation.statusCode}` : ""}`}</small></div>)}</div></div>)}</section><section><h3>聚合统计</h3><div className="table-wrap"><table><thead><tr><th>探测点</th><th>周期</th><th>样本</th><th>成功</th><th>不可用</th><th>平均延迟</th><th>开始</th></tr></thead><tbody>{evidence.stats.slice(0, 50).map((stat) => <tr key={stat.id}><td>{probes.find((probe) => probe.id === stat.probeId)?.name ?? shortId(stat.probeId)}</td><td>{stat.period === "hour" ? "小时" : "天"}</td><td>{stat.sampleCount}</td><td>{stat.successCount}</td><td>{stat.unavailableCount}</td><td>{stat.averageLatencyMs === null ? "-" : `${Math.round(stat.averageLatencyMs)} ms`}</td><td>{formatDate(stat.bucketStart)}</td></tr>)}</tbody></table></div></section></div>; }
