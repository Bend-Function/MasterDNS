...
function RotationTargetPicker({ open, accounts, onClose, onSelected }: { open: boolean; accounts: CloudAccount[]; onClose: () => void; onSelected: (selection: Selection) => void }) {
  ...
  return <Dialog open={open} title="选择轮换地址槽位" onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>取消</Button><Button disabled={!slot || loading} onClick={() => void submit()}>打开策略</Button></>}><div className="policy-form">{error && <div className="inline-error">{error}</div>}<Field label="云账号"><select value={accountId} onChange={(event) => void chooseAccount(event.target.value)}><option value="">选择账号</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Field><Field label="云实例"><select value={instanceId} disabled={!accountId} onChange={(event) => void chooseInstance(event.target.value)}><option value="">选择实例</option>{instances.map((item) => <option key={item.instance.id} value={item.instance.id}>{item.instance.name ?? item.instance.externalId} / {item.instance.region}</option>)}</select></Field><Field label="地址槽位"><select value={slotId} disabled={!instanceId} onChange={(event) => setSlotId(event.target.value)}><option value="">选择槽位</option>{slots.map((item) => <option key={item.slot.id} value={item.slot.id}>{item.slot.name} / IPv{item.slot.family} / {item.currentAddress?.address ?? "暂无观测地址"}{item.capability?.available ? "" : " / 不可自动轮换"}</option>)}</select></Field>{row && <div className="authorization-summary"><span>实例管理 <strong>{row.authorization?.managed ? "已授权" : "未授权"}</strong></span><span>IPv4 <strong>{row.authorization?.allowIpv4Rotation ? "已授权" : "关闭"}</strong></span><span>IPv6 <strong>{row.authorization?.allowIpv6Rotation ? "已授权" : "关闭"}</strong></span></div>}{loading && <LoadingState />}</div></Dialog>;
}
...
function waitingLabel(incident: RotationIncident) { if (incident.status === "paused") return "人工暂停，等待恢复"; if (incident.status === "exhausted") return "尝试次数已耗尽"; if (incident.errorCode) return incident.errorCode; if (incident.phase === "candidate") return "等待固定外部 Cohort 复测"; if (incident.phase === "cloud") return "等待云端读取确认"; if (incident.phase === "publish") return "等待 DNS 写入与远端验证"; if (incident.phase === "cleanup") return "等待清理期限与归属复核"; return "无需等待"; }
