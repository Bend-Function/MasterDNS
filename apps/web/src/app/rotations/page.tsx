...
import { cloudErrorMessage, capabilityReason, cloudProviderLabels, cloudRotationBlock, cloudServiceLabel, rotationDowntimeNotice } from "../../lib/cloud-ui";
...
export default function RotationsPage() {
  ...
  return <ConsoleLayout><PageHeader title="IP 自动轮换" description="按地址族管理策略，并追踪候选复测、DNS 发布与清理" actions={<><Button icon={<Settings2 size={14} />} onClick={() => setPickerOpen(true)}>选择地址槽位</Button><IconButton label="刷新轮换记录" onClick={() => void load()}><RefreshCw size={16} /></IconButton></>} />
    ...
    <Dialog open={selection !== null} title={confirmStart ? "确认手动启动轮换" : selection ? `${selection.slot.slot.name} / IPv${selection.slot.slot.family}` : "轮换策略"} size="large" onClose={close} footer={confirmStart ? <><Button variant="secondary" disabled={saving} onClick={() => { startIntent.current.cancel(); setConfirmStart(false); }}>返回</Button><Button variant="danger" icon={<RotateCw size={14} />} disabled={saving || !selection || cloudRotationBlock(selection.slot, selection.row.authorization) !== null} onClick={() => void start()}>{saving ? "提交中" : "确认启动"}</Button></> : <><Button variant="secondary" onClick={close}>关闭</Button><Button variant="secondary" icon={<RotateCw size={14} />} disabled={saving || !selection?.policy.enabled || cloudRotationBlock(selection.slot, selection.row.authorization) !== null} onClick={() => { startIntent.current.cancel(); setConfirmStart(true); }}>手动启动</Button><Button type="submit" form="rotation-policy-form" disabled={saving}>{saving ? "保存中" : "保存策略"}</Button></>}>
      {selection && (confirmStart ? <div className="danger-summary"><strong>本次操作可能修改真实云地址</strong><p>系统将复核当前管理授权、IPv{selection.slot.slot.family} 独立授权、外部健康证据和区域范围。新地址通过复测前不会发布 DNS；每次实际换址会消耗本次故障预算。</p>{rotationDowntimeNotice(selection.slot, selection.row.authorization?.allowReleaseAddress === true) && <p>{rotationDowntimeNotice(selection.slot, selection.row.authorization?.allowReleaseAddress === true)}</p>}<dl><dt>云服务</dt><dd>{cloudServiceLabel(selection.row.instance.service)} · {selection.row.instance.region}</dd><dt>当前实际地址</dt><dd className="mono">{selection.slot.currentAddress?.address ?? "暂无观测数据"}</dd><dt>最大尝试</dt><dd>{selection.policy.maxAttempts} 次</dd><dt>旧地址释放</dt><dd>{selection.row.authorization?.allowReleaseAddress ? "已授权" : "未授权"}</dd><dt>允许停止、启动或重启</dt><dd>{selection.row.authorization?.allowStopStart ? "已授权" : "未授权"}</dd></dl></div> : <RotationPolicyForm key={`${selection.slot.slot.id}:${selection.policy.revision}`} formId="rotation-policy-form" slot={selection.slot} authorization={selection.row.authorization} policy={selection.policy} onSubmit={savePolicy} />)}
    </Dialog>
  </ConsoleLayout>;
}
...
function RotationTargetPicker({ open, accounts, onClose, onSelected }: { open: boolean; accounts: CloudAccount[]; onClose: () => void; onSelected: (selection: Selection) => void }) {
  ...
  return <Dialog open={open} title="选择轮换地址槽位" onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>取消</Button><Button disabled={!slot || loading} onClick={() => void submit()}>打开策略</Button></>}><div className="policy-form">{error && <div className="inline-error">{error}</div>}<Field label="云账号"><select value={accountId} onChange={(event) => void chooseAccount(event.target.value)}><option value="">选择账号</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name} / {cloudProviderLabels[account.provider]}</option>)}</select></Field><Field label="云实例"><select value={instanceId} disabled={!accountId} onChange={(event) => void chooseInstance(event.target.value)}><option value="">选择实例</option>{instances.map((item) => <option key={item.instance.id} value={item.instance.id}>{item.instance.name ?? item.instance.externalId} / {cloudServiceLabel(item.instance.service)} / {item.instance.region}</option>)}</select></Field><Field label="地址槽位"><select value={slotId} disabled={!instanceId} onChange={(event) => setSlotId(event.target.value)}><option value="">选择槽位</option>{slots.map((item) => <option key={item.slot.id} value={item.slot.id}>{item.slot.name} / IPv{item.slot.family} / {item.currentAddress?.address ?? "暂无观测地址"}{item.capability?.available ? "" : " / 不可自动轮换"}</option>)}</select></Field>{slot && !slot.capability?.available && <div className="inline-warning">{capabilityReason(slot.capability?.reason)}</div>}{row && <div className="authorization-summary"><span>实例管理 <strong>{row.authorization?.managed ? "已授权" : "未授权"}</strong></span><span>IPv4 <strong>{row.authorization?.allowIpv4Rotation ? "已授权" : "关闭"}</strong></span><span>IPv6 <strong>{row.authorization?.allowIpv6Rotation ? "已授权" : "关闭"}</strong></span></div>}{loading && <LoadingState />}</div></Dialog>;
}
