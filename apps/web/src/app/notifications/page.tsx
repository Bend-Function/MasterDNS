  ...
  return <ConsoleLayout>
        ...
        {channels.loading ? <LoadingState /> : channels.error ? <ErrorState message={channels.error} /> : channels.data?.length === 0 ? <EmptyState title="没有通知渠道" /> : <div className="table-wrap"><table>
          ...
          <tbody>{channels.data?.map((channel) => <tr key={channel.id}>
            ...
            <td><div className="row-actions">
              ...
              <IconButton label="删除通知渠道" onClick={() => setDeleting(channel)}><Trash2 size={15} /></IconButton>
            </div></td>
          </tr>)}</tbody>
        </table></div>}
      </section>
    ...
    <Dialog open={editor !== null} title={editor === "new" ? "添加通知渠道" : "编辑通知渠道"} onClose={() => setEditor(null)} footer={<><Button variant="secondary" onClick={() => setEditor(null)}>取消</Button><Button type="submit" form="channel-form" disabled={busy}>保存渠道</Button></>}>
      <form id="channel-form" className="field-grid" onSubmit={save}>
        ...
        <label className="switch-row"><span>启用渠道</span><button type="button" className={`switch ${draft.enabled ? "on" : ""}`} aria-label="切换渠道状态" onClick={() => setDraft({ ...draft, enabled: !draft.enabled })} /></label>
        <label className="switch-row"><span>用户默认渠道</span><button type="button" className={`switch ${draft.isDefault ? "on" : ""}`} aria-label="切换默认渠道" onClick={() => setDraft({ ...draft, isDefault: !draft.isDefault })} /></label>
      </form>
    </Dialog>
    ...
    <Dialog open={linking !== null} title={`Pool 关联 · ${linking?.name ?? ""}`} size="large" onClose={() => setLinking(null)} footer={<Button onClick={() => setLinking(null)}>完成</Button>}>
      ...
      <form id="pool-channel-form" className="field-grid section-form" onSubmit={saveLink}>
        ...
        <label className="switch-row"><span>匹配时覆盖用户默认渠道</span><button type="button" className={`switch ${overridesDefaults ? "on" : ""}`} aria-label="切换默认渠道覆盖" onClick={() => setOverridesDefaults(!overridesDefaults)} /></label>
    ...
    <Dialog open={deleting !== null} title="删除通知渠道" size="small" onClose={() => setDeleting(null)} footer={<><Button variant="secondary" onClick={() => setDeleting(null)}>取消</Button><Button variant="danger" disabled={busy} onClick={() => void remove()}>删除渠道</Button></>}><p className="confirm-copy">删除 <strong>{deleting?.name}</strong> 及其 Pool 关联。已有投递历史将按数据库关系一并删除。</p></Dialog>
  </ConsoleLayout>;
}
