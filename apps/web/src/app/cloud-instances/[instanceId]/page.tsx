...
import { CloudInstanceLifecycle } from "../../../components/cloud-instance-lifecycle";
import { ManualRotationButton } from "../../../components/manual-rotation-button";
import { RelativeTime } from "../../../components/relative-time";
import { Button, Dialog, ErrorState, LoadingState, StatusBadge } from "../../../components/ui";
...
export default function CloudInstancePage() {
  ...
  const resetState = async () => {
    if (resetting) return;
    setResetting(true); setActionError(null); setNotice(null);
    try {
      if (UI_PREVIEW) { setResetOpen(false); setNotice("预览模式未重置本地状态"); return; }
      const result = await api<{ reset: boolean; resetIncidents: number; observedAddresses: string[]; awaitingVerification: boolean }>(`/v1/cloud-instances/${instanceId}/reset-state`, { method: "POST" });
      setResetOpen(false);
      await load();
      setNotice(`已同步云端地址并重置 ${result.resetIncidents} 个旧轮换流程。${result.awaitingVerification ? "新地址正在等待健康验证。" : ""}`);
    } catch (value) { setResetOpen(false); setActionError(value instanceof Error ? value.message : "本地状态重置失败"); }
  ...
  return <ConsoleLayout><div className="detail-header"><div className="detail-title"><Link className="icon-button" href="/cloud-instances" aria-label="返回云实例"><ArrowLeft size={17} /></Link><div><h1>{account?.name ?? instance.accountId} - {instance.name ?? instance.externalId}</h1><p>{cloudServiceLabel(instance.service)} · {instance.region} · <span className="mono">{instance.externalId}</span></p></div></div><div className="detail-actions">{account?.provider === "aws" && <Button variant="secondary" disabled={!account.enabled || !inScope || resetting} onClick={() => setResetOpen(true)}>同步云端并重置本地状态</Button>}<Button variant="secondary" icon={<RefreshCw size={14} />} onClick={() => void load()}>刷新</Button></div></div>
    ...
    <div className="content-grid"><div><section className="surface"><header className="surface-header"><div><h2>实际云资源</h2><p>最近一次完整清单读取结果</p></div><StatusBadge value={instance.state ?? "unknown"} /></header><div className="surface-body field-grid"><Info label="云账号" value={account ? `${account.name} · ${cloudProviderLabels[account.provider]}` : instance.accountId} /><Info label="远端 ID" value={instance.externalId} mono /><Info label="区域 / 可用区" value={`${instance.region}${typeof instance.metadata.availabilityZone === "string" ? ` / ${instance.metadata.availabilityZone}` : ""}`} /><Info label="清单状态" value={!present ? "远端已不存在" : !inScope ? "已排除在区域范围外" : account?.enabled === false ? "账号已停用" : "当前范围内"} /><Info label="最近发现" value={<RelativeTime value={instance.lastSeenAt} />} /><Info label="稳定资源 ID" value={instance.id} mono /></div></section>
      <CloudInstanceLifecycle key={`lifecycle:${instance.id}`} instance={instance} authorization={authorization} accountEnabled={Boolean(account?.enabled)} inScope={inScope} present={present} onChange={load} />
      <CloudTrafficCard key={`traffic:${instance.id}`} instanceId={instance.id} />
