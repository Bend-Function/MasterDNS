...
import { authorizationPayload, capabilityReason, cloudProviderLabels, cloudServiceLabel, rotationDowntimeNotice } from "../../../lib/cloud-ui";
...
export default function CloudInstancePage() {
  ...
  return <ConsoleLayout><div className="detail-header"><div className="detail-title"><Link className="icon-button" href="/cloud-instances" aria-label="返回云实例"><ArrowLeft size={17} /></Link><div><h1>{instance.name ?? instance.externalId}</h1><p>{cloudServiceLabel(instance.service)} · {instance.region} · <span className="mono">{instance.externalId}</span></p></div></div><div className="detail-actions"><Button variant="secondary" icon={<RefreshCw size={14} />} onClick={() => void load()}>刷新</Button></div></div>
