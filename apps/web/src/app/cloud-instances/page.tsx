...
import { cloudProviderLabels, cloudServiceLabel, loadVisibleInstanceAddresses } from "../../lib/cloud-ui";
...
export default function CloudInstancesPage() {
  ...
  const visible = useMemo(() => rows.filter(({ instance, account, addresses: observedAddresses }) => (!region || instance.region === region) && `${instance.name ?? ""} ${instance.externalId} ${instance.service} ${instance.region} ${account?.name ?? ""} ${observedAddresses?.map(address => address.address).join(" ") ?? ""}`.toLowerCase().includes(search.toLowerCase())), [region, rows, search]);
