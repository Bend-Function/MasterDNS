...
import { cloudAddressView, cloudInstanceMatches, cloudInventoryNotice, cloudProviderLabels, cloudServiceLabel, loadVisibleInstanceAddresses } from "../../lib/cloud-ui";
...
export default function CloudInstancesPage() {
  ...
  const visible = useMemo(() => rows.filter(row => (!region || row.instance.region === region) && cloudInstanceMatches(row, search, addresses[row.instance.id])), [region, rows, search, addresses]);
  ...
  return <ConsoleLayout><PageHeader title="云实例" description="AWS、Azure 与 Linode 实际清单、区域范围和显式管理授权" actions={<Button variant="secondary" icon={<RefreshCw size={14} />} onClick={() => void load()}>刷新清单</Button>} />
    <div className="toolbar"><div className="toolbar-left"><label className="search-box"><Search size={15} /><input aria-label="搜索云实例" placeholder="账号、机器名称、IP、服务或区域" value={search} onChange={(event) => setSearch(event.target.value)} /></label></div><div className="toolbar-right"><select aria-label="按区域筛选" value={region} onChange={(event) => setRegion(event.target.value)}><option value="">所有区域</option>{regions.map((value) => <option key={value}>{value}</option>)}</select></div></div>
    {loading ? <div className="surface"><LoadingState /></div> : error ? <div className="surface"><ErrorState message={error} onRetry={() => void load()} /></div> : visible.length === 0 ? <div className="surface"><EmptyState title="没有匹配的云实例" /></div> : <div className="table-wrap"><table><thead><tr><th>实例</th><th>账号</th><th>服务 / 区域</th><th>实际地址</th><th>远端状态</th><th>清单</th><th>管理授权</th><th>最近发现</th><th aria-label="操作" /></tr></thead><tbody>{visible.map((row) => { const { instance, authorization, inScope, account } = row;
      const present = instance.metadata.present !== false && row.inventory?.status !== "absent"; const available = Boolean(account?.enabled && inScope && present);
      const actual = addresses[instance.id] ?? (row.addresses?.length ? row.addresses : row.lastKnownAddresses ?? row.addresses);
      const addressView = cloudAddressView(actual ?? []);
      const inventoryNotice = cloudInventoryNotice(row.inventory, addressView.mode);
      return <tr key={instance.id}><td><Link className="table-primary" href={`/cloud-instances/${instance.id}`}><strong>{instance.name ?? instance.externalId}</strong><small className="mono">{instance.externalId}</small></Link></td><td><div className="table-primary"><strong>{account?.name ?? instance.accountId}</strong>{account && <small>{cloudProviderLabels[account.provider]}</small>}</div></td><td><div className="table-primary"><strong>{cloudServiceLabel(instance.service)}</strong><small>{instance.region}</small></div></td><td><div className="table-primary">{actual ? <>{addressView.addresses.map((address) => <small className="mono" key={address.id}>{address.address}{address.isCurrent === false ? " · 最近已知" : ""}</small>)}{inventoryNotice && <small>{inventoryNotice}</small>}</> : <Button variant="ghost" disabled={addressLoading === instance.id} onClick={() => void loadAddresses(row)}>{addressLoading === instance.id ? "加载中" : "加载实际地址"}</Button>}{addressErrors[instance.id] && <small className="inline-error">{addressErrors[instance.id]}</small>}</div></td><td><StatusBadge value={instance.state ?? "unknown"} /></td><td><StatusBadge value={available ? "active" : present ? "excluded" : "absent"} /></td><td><StatusBadge value={authorization?.managed ? "active" : "disabled"} /></td><td className="muted"><RelativeTime value={instance.lastSeenAt} /></td><td><Link className="icon-button" aria-label="查看实例详情" href={`/cloud-instances/${instance.id}`}><ExternalLink size={15} /></Link></td></tr>;
    })}</tbody></table></div>}
  </ConsoleLayout>;
}
