...
export async function withCloudRequest<T extends object>(
  tx: Transaction,
  identity: { key: string; actorUserId: string; ownerUserId: string; action: "account.create" | "slot.bind" | "rotation.start" | "rotation.manual" | "rotation.resume"; request: unknown },
  apply: () => Promise<T>,
