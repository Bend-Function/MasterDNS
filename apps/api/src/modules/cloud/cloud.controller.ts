import { Controller, Get, Param, ParseUUIDPipe, Patch, Query, Post } from "@nestjs/common";
...
export class CloudController {
  ...
  @Post("cloud-accounts") create(@CurrentUser() actor: AuthUser, @ZodBody(createCloudAccountSchema) input: CreateCloudAccountInput) { return this.cloud.create(actor, input); }
  ...
  @Post("address-slots/:id/bindings") bind(@CurrentUser() actor: AuthUser, @Param("id", ParseUUIDPipe) id: string, @ZodBody(cloudBindingSchema.omit({ slotId: true })) input: Omit<CloudBindingInput, "slotId">) { return this.bindings.bind(actor, { ...input, slotId: id }); }
}
