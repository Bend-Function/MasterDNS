...
export class CloudBindingsService {
  ...
  async bind(actor: AuthUser, input: CloudBindingInput) {
    try {
      return await this.queues.withDnsZoneLock(input.zoneId, (lease) => this.database.db.transaction(async (tx) => {
        ...
        if (!zone || zone.ownerUserId !== selected.ownerUserId) throw new NotFoundException("Zone and cloud slot must belong to the same owner");
        const fqdn = normalizeRecordName({ name: input.fqdn, type: input.recordType, content: source.address.address, ttl: 60, providerMetadata: {} }, zone.zone.nameAscii).name;
        ...
        if (input.takeoverExisting && (records.length !== 1 || record?.management !== "unmanaged" || record.content !== source.address.address)) throw new ConflictException("Takeover requires one unmanaged record matching the selected address");
        let pool: typeof endpointPools.$inferSelect;
        if (input.poolId) {
          const [existing] = await tx.select().from(endpointPools).where(and(eq(endpointPools.id, input.poolId), eq(endpointPools.ownerUserId, selected.ownerUserId))).for("update");
          if (!existing) throw new NotFoundException("Pool not found for cloud account owner");
          pool = existing;
        } else {
          [pool] = await tx.insert(endpointPools).values({ ownerUserId: selected.ownerUserId, name: fqdn.slice(0, 120), strategy: "primary_backup" }).returning() as [typeof endpointPools.$inferSelect];
        }
        ...
        await tx.insert(auditLogs).values({ ownerUserId: selected.ownerUserId, actorUserId: actor.id, source: "user", action: "cloud_binding.create", resourceType: "domain_binding", resourceId: binding.id, afterSnapshot: { binding, endpoint, slotId: source.slot.id } });
        lease.assertOwned();
