...
export class RotationService {
  ...
  async detail(actor: AuthUser, id: string) {
    ...
    const [segments, attempts, steps, resources, publications] = await Promise.all([
      ...
      this.database.db.select({ id: rotationResources.id, addressId: rotationResources.addressId, attemptId: rotationResources.attemptId, address: rotationResources.address, role: rotationResources.role, origin: rotationResources.origin, cleanupStatus: rotationResources.cleanupStatus, cleanupDueAt: rotationResources.cleanupDueAt, cleanupError: rotationResources.cleanupError, cleanupStepId: rotationResources.cleanupStepId }).from(rotationResources).where(eq(rotationResources.incidentId, id)),
