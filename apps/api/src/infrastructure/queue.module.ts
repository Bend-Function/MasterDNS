...
export class QueueService implements OnModuleDestroy {
  ...
  readonly rotation = new Queue(queueNames.rotation, { connection: this.redis });
  ...
  async onModuleDestroy() {
    await Promise.all([this.operations.close(), this.health.close(), this.reconcile.close(), this.sync.close(), this.cloudSync.close(), this.rotation.close(), this.notifications.close()]);
