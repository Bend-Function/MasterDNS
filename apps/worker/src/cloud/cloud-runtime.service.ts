...
export class CloudRuntimeService {
  ...
  async adapter(accountId: string, service: "ec2" | "lightsail"): Promise<CloudAdapter> {
    const [account] = await this.database.db.select().from(cloudAccounts).where(eq(cloudAccounts.id, accountId)).limit(1);
    if (!account?.enabled) throw new CloudError("permission_denied", false);
    ...
    await this.database.db.transaction(async (tx) => {
      ...
      if (!current?.enabled || current.credentialCiphertext !== account.credentialCiphertext) throw new CloudError("permission_denied", false);
