...
export async function fixture(db: ReturnType<typeof createDatabase>["db"], mode: "static" | "ddns" = "static") {
  ...
  const agents = await db.insert(probeAgents).values([1,2].map(i => ({ ownerUserId: user!.id, name: `probe${i}`, capabilities: { ipv4: true, ipv6: true }, lastSeenAt: new Date("2026-09-15T01:00:00Z") }))).returning();
