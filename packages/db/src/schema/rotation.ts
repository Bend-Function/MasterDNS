...
export function defineRotationSchema(dependencies: Dependencies) {
  ...
  const rotationIncidents = pgTable("rotation_incidents", {
    ...
    healthPolicyId: uuid("health_policy_id"),
    healthPolicyRevision: integer("health_policy_revision"),
  ...
  const rotationResources = pgTable("rotation_resources", {
    ...
    cleanupStepId: varchar("cleanup_step_id", { length: 180 }).references(() => rotationSteps.id, { onDelete: "restrict" }),
    cleanupAddressVersion: integer("cleanup_address_version"),
  ...
  const rotationPublications = pgTable("rotation_publications", {
    ...
    children: jsonb("children").$type<Array<{ poolId: string; eventId: string; policyRevision: number; decisionRevision: number; operationId?: string }>>().notNull().default([]),
    context: jsonb("context").$type<Record<string, unknown>>(),
    previousMaxTtl: integer("previous_max_ttl").notNull().default(0),
    promotedAt: time("promoted_at"),
