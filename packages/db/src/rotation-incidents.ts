...
import { addressHealthPolicies, addressHealthStates, auditLogs, healthCheckConfigs, probeGroups, rotationAttempts, rotationBudgetSegments, rotationIncidents, rotationLeases, rotationSteps } from "./schema/index.js";
...
export async function resumeRotationIncident(tx: RotationTransaction, c: RotationContext, id: string, actorUserId: string) {
  ...
  const error = rotationAuthorizationError(c); if (error) throw new Error(error);
  const h = await lockRotationHealth(tx, c); const revisions = healthRevisions(h);
  ...
  const [updated] = await tx.update(rotationIncidents).set({ status: "active", currentSegmentId: continuingCloud ? incident.currentSegmentId : segmentId, pendingSegmentId: continuingCloud ? segmentId : null, ...(incident.phase === "cloud" && !continuingCloud ? { currentAttemptId: null } : {}), pausedByUserId: null, errorCode: null,
