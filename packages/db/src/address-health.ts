...
export type HealthTargetIdentity = { slotId?: string | null | undefined; endpointId?: string | null | undefined; family: "4" | "6" };
export function healthTargetWhere(table: typeof addressHealthStates, target: HealthTargetIdentity) {
  return target.slotId ? eq(table.slotId, target.slotId) : and(eq(table.endpointId, target.endpointId!), eq(table.family, target.family));
}
...
export async function lockHealthTarget(tx: ProbeTransaction, target: HealthTargetIdentity, initialize = false, now = new Date()) {
  if (target.slotId) {
    ...
    if (!slot || !address || slot.family !== target.family) return undefined;
    return { slot, addressId: address.id, address: address.address, family: slot.family, addressVersion: slot.candidateAddressId ? slot.candidateVersion : slot.currentVersion };
  }
  ...
  const address = addresses.find(a => a.state === "candidate") ?? addresses.find(a => a.state === "current");
  if (!endpoint || !pool || !address || endpoint.lifecycle !== "enabled" || endpoint.addressMode === "cloud") return undefined;
  return { endpoint, pool, endpointAddress: address, addressId: address.id, address: address.address, family: address.family, addressVersion: 1 };
}
